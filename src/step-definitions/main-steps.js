...
When(/^the user selects the (.+) sort option$/, async (option) => {
  ...
  const selected = mainPage.sideFilters.sortDropdown.$(
    `//option[text()="${option}"]`
  );
...
Then(
  /^products should be displayed in descending order of price$/,
  async () => {
    const list = await mainPage.productList.rootElement;
    await list.waitUntil(
      async function () {
        return (await this.getAttribute('data-test')) === 'sorting_completed';
      },
      {
        timeoutMsg:
          'Expected product list to have data-test="sorting_completed"',
      }
    );
...
Then(
  /^the search results should display all product whose names contain (.+)$/,
  async (query) => {
    const searchResult = await mainPage.productList.rootElement;
    await searchResult.waitUntil(
      async function () {
        return (await this.getAttribute('data-test')) === 'search_completed';
      },
      {
        timeoutMsg:
          'Expected search result to have data-test="search_completed"',
      }
    );
